...
function showConvCtxMenu(e, convId, itemEl) {
  ...
  pinBtn.textContent  = _pinnedConvs.has(convId) ? '📌 Desafixar' : '📌 Fixar';
  muteBtn.textContent = _mutedConvs.has(convId)  ? '🔔 Ativar notificações' : '🔕 Desativar notificações';
...
function showMsgCtxMenu(e, msg, isOut, rowEl) {
  ...
  menu.querySelector('[data-action="favorite"]').textContent = isFav ? '★ Remover dos favoritos' : '☆ Favoritar';
  menu.querySelector('[data-action="pin"]').textContent = msg.is_pinned ? '📌 Desafixar' : '📌 Fixar';
...
function openFavoritesPanel() {
  ...
  if (!_favorites.length) {
    list.innerHTML = '<div class="empty-state-sidebar"><span>★</span><p>Nenhuma mensagem favoritada</p></div>';
  } else {
...
function showArchivedList() {
  // Filtra conversas arquivadas e mostra como lista temporária
  const archived = _conversations.filter(c => _archivedConvs.has(c.id));
  if (!archived.length) { toast('Nenhuma conversa arquivada'); return; }
  toast(`${archived.length} conversa(s) arquivada(s) — em breve uma tela dedicada`);
}
...
function showContactProfile(user) {
  ...
  const isBlocked = _blockedUsers.has(user.id);
  document.getElementById('cp-block').textContent = isBlocked ? '🚫 Desbloquear' : '🚫 Bloquear';
