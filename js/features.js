...
let _pendingArchiveConvId = null; // conv aguardando arquivamento após senha
let _awaitingNewPassword  = false; // FIX #3: controla fluxo de 2 etapas na troca de senha
...
function openArchiveEntry() {
  _awaitingNewPassword = false; // FIX #3: reseta estado ao abrir modal
  if (_archiveHash) {
    const modal = document.getElementById('archive-password-modal');
    document.getElementById('archive-password-input').value = '';
    document.getElementById('archive-password-input').placeholder = 'Senha';
...
async function handleArchiveSubmit() {
  // FIX #3: sem senha definida, bloqueia entrada — usuário deve criar senha primeiro
  if (!_archiveHash) {
    toast('Crie uma senha primeiro usando o botão abaixo', 'error');
    return;
  }
  const val  = document.getElementById('archive-password-input').value;
  if (!val) return;
  const hash = await sha256(val);
  if (hash === _archiveHash) {
    document.getElementById('archive-password-modal').classList.add('hidden');
    showArchivedList();
  } else {
...
async function handleSetArchivePassword() {
  ...
  // FIX #3: se já existe senha, exige verificar a senha antiga antes de definir nova
  if (_archiveHash && !_awaitingNewPassword) {
    const hash = await sha256(val);
    if (hash !== _archiveHash) {
    ...
    // Senha antiga correta — pede a nova
    _awaitingNewPassword = true;
    document.getElementById('archive-password-input').value = '';
    document.getElementById('archive-password-input').placeholder = 'Nova senha (mín. 4 chars)';
    toast('Senha verificada! Agora digite a nova senha.');
    return;
  }

  // Define ou redefine a senha
  _awaitingNewPassword = false;
  document.getElementById('archive-password-input').placeholder = 'Senha';
  const hash = await sha256(val);
  ...
  // Arquiva a conversa pendente se houver
  if (_pendingArchiveConvId) {
    const id = _pendingArchiveConvId;
    _pendingArchiveConvId = null;
    await _doArchiveConv(id);
  }
}

async function archiveConv(convId) {
  // Bug 1: se não tem senha definida, pede para criar uma antes de arquivar
  if (!_archiveHash) {
    _pendingArchiveConvId = convId;
    const modal = document.getElementById('archive-password-modal');
    document.getElementById('archive-password-input').value = '';
    document.getElementById('archive-set-password-label').textContent = 'Criar senha para arquivos';
    modal.classList.remove('hidden');
    document.getElementById('archive-password-input').focus();
    return;
  }
  _doArchiveConv(convId);
}

async function _doArchiveConv(convId) {
...
// ══════════════════════════════════════════════════
// FIX #14: agora deleta do Supabase também (antes só removia do array local)
function confirmDeleteConv(convId) {
  ...
  renderConversationList();
  if (_activeConvId === convId) hideConversation();
  if (_supabase) deleteConversationDB(convId).catch(() => {});
...
// ══════════════════════════════════════════════════
// FIX #13: seletor correto é '.unread-dot' (não '.chat-item-unread')
// e atualiza last_read no localStorage para não voltar ao renderizar
function markAllConvsRead() {
  document.querySelectorAll('.unread-dot').forEach(el => el.remove());
  const now = new Date().toISOString();
  _conversations.forEach(conv => {
    localStorage.setItem(`last_read:${conv.id}`, now);
  });
