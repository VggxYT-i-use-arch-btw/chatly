 ...
 */
async function startRecording(vizCanvas, timerEl) {
  _recChunks = [];
  _recSecs   = 0;
 ...
 */
async function extractPeaks(blob, numPeaks = 80) {
 ...
 */
function drawWaveform(canvas, peaks, progress = 0, isOut = true) {
  const ctx = canvas.getContext('2d');
 ...
 * @param {boolean} isOut
 */
async function togglePlay(id, url, canvas, peaks, btnEl, timeEl, isOut) {
  ...
  btnEl.innerHTML = '<span class="material-symbols-rounded">pause</span>';
  ...
  try {
    ...
    btnEl.innerHTML = '<span class="material-symbols-rounded">play_arrow</span>';
  }
}
...
function playFrom(id, state, canvas, peaks, offset) {
  ...
  src.onended = () => {
    if (_playing.get(id) === state && state.playing) {
      ...
      state.playing = false;
      state.btn.innerHTML = '<span class="material-symbols-rounded">play_arrow</span>';
      state.offset  = 0;
...
function pause(id, state, canvas, peaks, btnEl, timeEl, isOut) {
  ...
  if (btnEl)   btnEl.innerHTML = '<span class="material-symbols-rounded">play_arrow</span>';
