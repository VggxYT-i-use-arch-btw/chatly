...
// ── PROFILE PANEL UI ─────────────────────────────
let _profileSb = null;
function injectSupabase(sb) { _profileSb = sb; }
...
function renderMiniAvatar(p = _profile) {
  ...
  const letter = document.getElementById('my-avatar-letter');
  if (p.avatar_data) {
    ...
    letter.style.cssText = 'font-size:15px;font-weight:700;font-family:Outfit,sans-serif;';
    letter.textContent = (p.display_name || '?')[0].toUpperCase();
  }
}
