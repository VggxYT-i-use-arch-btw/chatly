...
import { getLocalProfile, saveLocalProfile, dbPut, dbGet } from './db.js';
 ...
 */
export function compressImage(file, maxSize = 300, quality = 0.82) {
...
export function usernameNextChangeMs() {
...
// ── PROFILE PANEL UI ─────────────────────────────
let _supabase = null;
export function injectSupabase(sb) { _supabase = sb; }
...
export function renderMiniAvatar(p = _profile) {
  if (!p) return;
...
// ── SYNC TO SUPABASE ──────────────────────────────
export async function syncProfileToSupabase(supabase, profile) {
  if (!supabase) return;
  try {
