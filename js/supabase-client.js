...
// Busca somente a última mensagem de uma conversa (para preview na lista)
...
async function sendMessage({
  ...
  mediaSize  = null,
  audioPeaks = null,
  replyToId  = null,   // FIX #8: reply_to_id agora é passado corretamente
}) {
  ...
  const { data, error } = await getSb()
    ...
    .insert({
      ...
      reply_to_id:     replyToId  || null,  // FIX #8
    })
    .select()
    .single();
...
async function softDeleteProfileDB(userId) {
  // FIX #5: username também é randomizado para ficar disponível novamente
...
// FIX #14: deleta conversa do Supabase de verdade
async function deleteConversationDB(convId) {
  const { error } = await getSb()
    .from('conversations')
    .delete()
    .eq('id', convId);
  if (error) console.warn('[deleteConversationDB]', error.message);
}
