...
function applyBubbleColor(color) {
  ...
  root.style.setProperty('--bg-bubble-out',    color);   // variável usada pelo CSS das bolhas
  ...
  root.style.setProperty('--fab-shadow',        color + '80');   // glow do FAB (50% alpha)
  root.style.setProperty('--fab-shadow-hover',  color + '99');   // glow hover (60% alpha)
...
function applyWallpaper(id) {
  // Aplica no .conversation (pai do scroll) para o fundo ficar fixo
  const target = document.querySelector('.conversation') || document.getElementById('messages-scroll');
  if (!target) return;

  // Limpa scroll também caso tenha ficado estilo antigo
  ...
  if (id === 'custom' && _customWallpaperUrl) {
    target.style.backgroundImage    = `url(${_customWallpaperUrl})`;
  ...
  if (!wp || !wp.css) {
    ...
    target.style.backgroundImage    = wp.css;
    target.style.backgroundSize     = wp.size || 'auto';
    target.style.backgroundPosition = 'center';
    target.style.backgroundAttachment = 'local';
  }
}
...
async function executeDeleteAccount() {
  ...
  // FIX #19: inclui o store 'stickers' que estava faltando
  const db = await openDB();
  const tx = db.transaction(['profile','conversations','messages','blobcache','stickers'], 'readwrite');
  ['profile','conversations','messages','blobcache','stickers'].forEach(s => tx.objectStore(s).clear());
  ...
  // FIX #20: remove apenas as chaves do Chatly — não afeta outros apps no mesmo domínio
  const chatlyPrefixes = ['last_read:', 'deleted_for_me:', 'cleared_at:', 'pref:'];
  const keysToRemove = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (k && chatlyPrefixes.some(prefix => k.startsWith(prefix))) {
      keysToRemove.push(k);
    }
  }
