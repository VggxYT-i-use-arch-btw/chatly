...
// FIX #16: rastreia mensagens otimistas em voo para evitar duplicatas no realtime
const _pendingMsgKeys = new Map(); // `convId:senderId:content` → el
...
async function renderConversationList() {
  ...
  // Filtra arquivadas e ordena: fixadas primeiro
  const visible = _conversations
    .filter(c => !isConvArchived(c.id))
    .sort((a, b) => {
      const ap = isConvPinned(a.id) ? 1 : 0;
      const bp = isConvPinned(b.id) ? 1 : 0;
  ...
  if (!visible.length) {
    list.innerHTML = `
      <div class="empty-state-sidebar">
        <span class="material-symbols-rounded" style="font-size:48px;color:#8696a0">chat</span>
        <p>Sem conversas ainda</p>
        <small>Toque em <span class="material-symbols-rounded" style="font-size:14px;vertical-align:middle">add_comment</span> para começar</small>
      </div>`;
    return;
  }
  ...
  for (const conv of visible) {
    ...
    // Bug 3 fix: busca a ÚLTIMA mensagem (descending) para preview correto
    const last = _supabase ? await fetchLastMessage(conv.id).catch(() => null) : null;

    const item = createChatItemEl(conv, otherUser, last, profile.id, () => {
      openConversation(conv, otherUser);
    });

    // Bug 4: indicador de não lidas
    if (last && last.sender_id !== profile.id) {
      const lastRead = localStorage.getItem(`last_read:${conv.id}`);
      const hasUnread = !lastRead || new Date(last.created_at) > new Date(lastRead);
      if (hasUnread) {
    ...
    if (isConvPinned(conv.id)) {
      item.classList.add('conv-pinned');
      // FIX #4: adiciona ícone de alfinete visível na hora
      const timeEl = item.querySelector('.chat-item-time');
      if (timeEl) {
        const pin = document.createElement('span');
        pin.className = 'material-symbols-rounded';
        pin.style.cssText = 'font-size:13px;vertical-align:middle;margin-right:2px;opacity:0.7;';
        pin.textContent = 'push_pin';
        timeEl.insertBefore(pin, timeEl.firstChild);
      }
    }
    if (isConvMuted(conv.id))  item.classList.add('conv-muted');

    attachConvContextMenu(item, conv.id);
...
async function openConversation(conv, otherUser) {
  ...
  // Header — FIX #11: grupos mostram nome e contagem de participantes
  const cAvImg  = document.getElementById('contact-avatar-img');
  const cAvLet  = document.getElementById('contact-avatar-letter');

  if (conv.is_group) {
    document.getElementById('contact-display-name').textContent = conv.group_name || 'Grupo';
    const count = conv.participants?.length ?? 0;
    document.getElementById('contact-lastseen').textContent = `${count} participante${count !== 1 ? 's' : ''}`;

    const groupAvatarUrl = conv.group_avatar_url || null;
    if (groupAvatarUrl) {
      cAvImg.src    = groupAvatarUrl;
      cAvImg.hidden = false;
      cAvLet.hidden = true;
    } else {
      cAvImg.removeAttribute('src');
      cAvImg.hidden = true;
      cAvLet.hidden = false;
      cAvLet.classList.remove('material-symbols-rounded');
      cAvLet.style.cssText = 'font-size:15px;font-weight:700;font-family:Outfit,sans-serif;';
      cAvLet.textContent = ((conv.group_name || 'G')[0]).toUpperCase();
    }
  } else {
    document.getElementById('contact-display-name').textContent = otherUser?.display_name || 'Usuário';
    document.getElementById('contact-lastseen').textContent     = '@' + (otherUser?.username || '—');
  ...
  // Bug 4: marca conversa como lida
  localStorage.setItem(`last_read:${conv.id}`, new Date().toISOString());
...
function hideConversation() {
  // Bug 6: no mobile a welcome-screen fica escondida via CSS, não precisa aparecer
  if (!_isMobile) {
...
function renderMessages(msgs) {
  ...
  container.innerHTML = '';
  const profile = getProfile();
  // FIX #10: filtra mensagens deletadas para mim
  const filtered = msgs.filter(m => !isMsgDeletedForMe(m.id, profile.id));
  ...
  for (const item of groups) {
    if (item.type === 'divider') {
      ...
      const msg = item.data;
      const isOut = msg.sender_id === profile.id;
      const el = createMessageEl(msg, profile.id, openMediaModal);
      attachMsgContextMenu(el, msg, isOut);
      // Link clickável
      if (msg.type === 'text' && msg.content) linkifyMsgEl(el);
      container.appendChild(el);
    }
  }
  scrollToBottom();
  // Mostra banner de fixado se houver
  if (_pinnedMessages[_activeConvId]) showPinnedBanner(_pinnedMessages[_activeConvId]);
...
function onNewMessage(msg) {
  ...
  // FIX #10: ignora mensagens deletadas para mim
  if (isMsgDeletedForMe(msg.id, profile.id)) return;

  // FIX #16: se mensagem chegou via realtime antes do await do sendMessage resolver,
  // atualiza o elemento otimista em vez de criar duplicata
  const pendingKey = `${msg.conversation_id}:${msg.sender_id}:${msg.content}`;
  if (_pendingMsgKeys.has(pendingKey)) {
    const optEl = _pendingMsgKeys.get(pendingKey);
    optEl.dataset.id = msg.id;
    _pendingMsgKeys.delete(pendingKey);
    return;
  }

  // Evita duplicar se a mensagem já está no DOM (ex: confirmação normal do servidor)
...
async function handleSendText() {
  ...
  attachMsgContextMenu(el, optimistic, true);
  linkifyMsgEl(el);
  document.getElementById('messages-inner').appendChild(el);
  scrollToBottom();

  // Bug 5: tick simples enquanto não confirmado
  const tickEl = el.querySelector('.msg-tick');
  if (tickEl) tickEl.textContent = 'done';

  // FIX #16: registra como mensagem pendente antes do await
  const pendingKey = `${_activeConvId}:${profile.id}:${text}`;
  _pendingMsgKeys.set(pendingKey, el);
  ...
  try {
    if (_supabase) {
      const saved = await sendMessage({
        ...
        content:   text,
        type:      'text',
        replyToId: replyTo?.id || null,  // FIX #8: envia reply_to_id para o banco
      });
      // Atualiza ID e tick para "enviado"
      el.dataset.id = saved.id;
      if (tickEl) tickEl.textContent = 'done_all';
    }
  } catch (err) {
    toast('Erro ao enviar mensagem', 'error');
    if (tickEl) { tickEl.textContent = 'error'; tickEl.style.color = '#ff5252'; }
  } finally {
    _pendingMsgKeys.delete(pendingKey); // FIX #16: limpa mesmo em caso de erro
  }
}
...
//  NEW CHAT
// ══════════════════════════════════════════════════
// openNewChatDialog é definida em groups.js com suporte a tabs e grupos
...
// ══════════════════════════════════════════════════
//  LINKIFY (torna URLs clicáveis em mensagens de texto)
// ══════════════════════════════════════════════════
// FIX #15: linkify ANTES de escapar HTML — evita que & em URLs vire &amp; e quebre o regex
function linkifyMsgEl(rowEl) {
  const p = rowEl.querySelector('.msg-text');
  if (!p) return;
  const text = p.textContent;
  const urlRe = /(https?:\/\/[^\s]+)/g;
  if (!urlRe.test(text)) return;
  urlRe.lastIndex = 0;
  // split com grupo capturador: índices ímpares = URLs, pares = texto puro
  const parts = text.split(urlRe);
  p.innerHTML = parts.map((part, i) => {
    if (i % 2 === 1) {
      const safeUrl = escHtml(part);
      return `<a href="${safeUrl}" target="_blank" rel="noopener noreferrer" class="msg-link">${safeUrl}</a>`;
    }
    return escHtml(part);
  }).join('');
}
