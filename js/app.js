...
async function renderConversationList() {
  ...
  for (const conv of visible) {
    ...
    // Indicadores
    if (isConvPinned(conv.id)) item.classList.add('conv-pinned');
...
function onNewMessage(msg) {
  ...
  // Evita duplicar se a mensagem já está no DOM (enviada por mim via otimista)
  if (document.querySelector(`[data-id="${msg.id}"]`)) return;
  // Mantém cache atualizado para swipe-reply e painel de mídia
...
function linkifyMsgEl(rowEl) {
  ...
  p.innerHTML = escHtml(text).replace(
    /(https?:\/\/[^\s]+)/g,
