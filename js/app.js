...
import { initSupabase, supabase, upsertProfile, fetchConversations,
         findOrCreateConversation, fetchMessages, sendMessage,
         subscribeToConversation, fetchUserByUsername, fetchUserById,
         isUsernameAvailable } from './supabase-client.js';

import { loadProfile, getProfile, setProfile, compressImage,
         sanitizeUsername, isValidUsername, canChangeUsername,
         formatCooldownRemaining, renderProfilePanel, renderMiniAvatar,
         injectSupabase } from './profile.js';

import { requestPersistentStorage, dbGet, dbPut } from './db.js';
import { uploadToR2, detectMediaType, formatFileSize } from './r2.js';
import { startRecording, stopRecording, cancelRecording, isRecording, extractPeaks } from './audio.js';
import { createMessageEl, createChatItemEl, createAvatarEl,
         groupMessagesByDate, formatDateDivider } from './chat.js';
...
async function boot() {
  ...
  try {
    ...
    toast('Configure as credenciais em js/config.js para sincronizar com o servidor', 'error');
  }
  ...
  if (!profile) {
    ...
    showApp();
    if (_supabase) {
      await upsertProfile(profile).catch(e => console.warn('[Boot] upsert:', e.message));
    }
  }
}
